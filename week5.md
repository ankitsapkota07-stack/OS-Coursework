...
```
Screenshot:
(images/week-5_armor_status.png)

Key Findings:

Multiple AppArmor profiles are loaded

Profiles are running in mixed modes (complain, enforce, kill)

System services like snap applications have defined profiles
...
Screenshot:
https://week-5-entire-terminal.png

Key Findings:

Unattended-upgrades package successfully installed

Configuration file exists at /etc/apt/apt.conf.d/50unattended-upgrades

Security and updates repositories are configured for automatic upgrades
...
Screenshots:
https://week-5-install-fail2ban.png
https://week-5-config-edits.png
https://week-5-jail-status.png

Key Findings:

Fail2ban successfully installed with dependencies

SSH jail enabled and monitoring /var/log/auth.log

Currently no banned IPs (clean system state)
...
./security-workstation.sh
Screenshot:
https://week-5-scrnshot.png

Key Findings:

SSH connection successful via port 2222

SSH service active and running

Custom SSH port 2222 confirmed in configuration

Firewall status retrieved (requires sudo on server)

Automatic updates configuration verified

Fail2ban status checked successfully
...
./monitor-server.sh
Screenshot:
https://week-5-monitor-server.png
